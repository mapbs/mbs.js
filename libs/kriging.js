...
var kriging = function() {
     ...
     */
    FastPip = function(width, height, polygon){
    ...
    // fast grid
    kriging.fastGrid = function(polygons, variogram, width) {
        var i, j, k, n = polygons.length; // n 多边形的个数
        if(n==0) return;
        // Boundaries of polygons space
        var xlim = [polygons[0][0][0], polygons[0][0][0]];
        var ylim = [polygons[0][0][1], polygons[0][0][1]];
        ...
        for(i=0;i<=x;i++) A[i] = Array(y+1);
        for(i=0;i<n;i++) {
            // Range for polygons[i] 计算出第i个多边形的边界矩形
            lxlim[0] = polygons[i][0][0];
            lxlim[1] = lxlim[0];
            lylim[0] = polygons[i][0][1];
            lylim[1] = lylim[0];
            ...
            // 对每个多边形生成一个FastPip实例
            // 将多边形坐标转换为网格点坐标
            const transPolygon = polygons[i].map(([x, y]) => {
                let i = Math.floor((x - xlim[0]) / width)
                let j = Math.floor((y - ylim[0]) / width)
                return [i, j]
            })
            const fastPip = FastPip(a[1], b[1], transPolygon)
...
//		var index = Math.floor((colors.length-1)*z);
//		if(index==0){
//			continue;
//		}
//		console.log(z +"   " + colors[Math.floor((colors.length-1)*z)]);
                ctx.fillStyle = colors[Math.floor((colors.length-1)*z)];
//		if(grid[i][j]<100){
//			continue;
//		}
//		console.log(grid[i][j])
//		ctx.fillStyle = this.getColor(colors,grid[if][j]);
